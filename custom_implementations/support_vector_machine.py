...
### Performance Comparison ("Train a simple model with Concrete ML")

# scikit learn
svm_sklearn = SklearnLinearSVC(max_iter=100)
svm_sklearn.fit(X_train, y_train)
# plot the boundary
plot_decision_boundary(svm_sklearn, X_test, y_test, title="Scikit Learn SVM Decision Boundary")

# concrete_ml FHE STUFF
svm_concrete = ConcreteLinearSVC(max_iter=100, n_bits=8)
svm_concrete.fit(X_train, y_train)
# plot the boundary
plot_decision_boundary(svm_concrete, X_test, y_test, title="ConcreteML SVM Decision Boundary")